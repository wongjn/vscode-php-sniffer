...
const { FileSystemError, Uri, window, workspace } = require('vscode');
...
const createRunner = (token, uri, fullDocument = true) => {
  ...
  return {
    ...
    async phpcs(stdin) {
      // Ensure correct column numbers in report messages when using tab
      // indentation: tabs count as one character for vscode.Position.
      if (window.activeTextEditor && !window.activeTextEditor.options.insertSpaces) {
        args.set('tab-width', '1');
      }
