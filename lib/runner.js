...
const { RelativePattern, workspace } = require('vscode');
...
async function resolveRunFolder(config, folder) {
  ...
  if (folder && config.get('autoDetect') && !result) {
    const files = await workspace.findFiles(new RelativePattern(folder, 'vendor/bin/{phpcs,phpcbf}'), undefined, 1);
    if (files.length) {
      result = `${dirname(files[0].fsPath)}${sep}`;
    }
  }
