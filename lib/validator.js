...
const { languages, window, workspace, CancellationTokenSource, ProgressLocation } = require('vscode');
...
const validateDocument = (diagnostics, tokenManager) => (document) => {
  ...
  // Pass tab width to phpcs if indenting by tabs so that one can reliably
  // compensate for column number (in phpcs reports) vs character number
  // for diagnostics.
  const editorOptions = window.activeTextEditor && window.activeTextEditor.options;
  /** @type {number|false} */
  const tabWidth = (editorOptions) ? /** @type {number} */ (editorOptions.tabSize) : false;

  const text = document.getText();
  const resultPromise = createRunner(token, document.uri).phpcs(text, tabWidth);
  ...
  resultPromise
    .then((result) => {
      if (document.isClosed) {
        ...
        diagnostics.set(document.uri, reportFlatten(result, text, tabWidth || 1));
      }
    })
    .catch((error) => {
