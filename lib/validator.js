...
/**
 * Returns whether a given document matches the `extraFiles` setting.
 *
 * @param {import('vscode').WorkspaceConfiguration} config
 *   Settings for `phpSniffer`.
 ...
 */
const matchExtraFiles = (config, document) => {
  const selectors = config
    .get('extraFiles', [])
    .map((pattern) => ({ pattern, scheme: 'file' }));
  return languages.match(selectors, document) > 0;
};
...
const shouldValidate = (document) => {
  ...
  return (
    !document.isClosed
    && (document.languageId === 'php' || matchExtraFiles(config, document))
