...
/**
 * Determines whether a number is more than another.
 *
 * @param {number} limit
 *   The baseline.
 *
 * @return {(count: number) => boolean}
 *   Returns true if the given count is more than the baseline.
 */
const moreThan = (limit) => (count) => count > limit;
...
const getIndentation = (indent) => pipe(
  reduceWhile(moreThan(0), lintIndentReducer(lineIndentCounter(indent)), Number.MAX_SAFE_INTEGER),
  stringRepeat(indent),
);
